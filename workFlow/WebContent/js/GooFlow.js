...
//初始化区域图的对象
	...
	this.$nodeDom={};
	this.$areaDom = {};
	this.$customData = [];//用来临时存储用户自定义数据,如果从数据库中读取数据，也可以在页面上赋值过来
	...
	if(property.haveHead){
		...
		this.New=null;//刷新流程图按钮被点中
		this.Load=null;//加载流程图文件按钮定义
		this.Down=null;//保存流程图按钮定义
		this.Read=null;//图形化流程图定义
		this.Switch=null;//切换模式定义
		this.Switch2=null;
		...
		this.$head.on("click",{inthis:this},function(e){
			...
			switch($(tar).attr("class")){
				case "ico_new":	if(This.New!=null)	This.New();break;
				case "ico_down":	if(This.Down!=null)	This.Down();break;
				case "ico_undo":	This.undo();break;
				case "ico_redo":	This.redo();break;
				case "ico_load":    if(This.Load!=null)	This.Load();break;
				case "ico_read":    This.Read();break;
				case "ico_switch": This.Switch();break;
				case "ico_switch2": This.Switch2();break;
				
				case "ico_test1": This.saveMode();break;
				case "ico_test2": This.downloadMode();break;
				case "ico_test3": This.selectMode();break;
				case "ico_test4": This.readMode();break;
			}
		});
	}
	var toolWidth=0;
	...
	width=width-toolWidth-25;//调整画布距离左侧远近！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
...
GooFlow.prototype={
	...
	initDraw:function(id,width,height){
		...
		$(this.$draw).delegate(tmpClk,"dblclick",{inthis:this},function(e){
		    ...
		    //重写线条的双击事件
			var tpstr = This.$lineData[this.id].type;
			var json = This.GetCumstomData(myid);
			json.hjlx = tpstr;
			var content = This.GetLineNodeContent(myid, json);
			art.dialog({
			    title: "设置流转条件",
			    lock: true,
			    content: content,
			    ok: function () {
			        var newTxt = $("#txtHjname").val();
			        json.hjname = newTxt;
			        var sarr = [];
			        var lilist = $("ul.mylist li");
			        if (lilist.length > 0) {
			            for (var i = 0; i < lilist.length; i++) {
			                var jsn = {};
			                var span = lilist.eq(i).find("span");
			                if (span.length > 0) {
			                    jsn.bh = span.attr("uno");
			                    jsn.condition = span.text();
			                }
			                sarr.push(jsn);
			            }
			        }
			        json.conlist = sarr;
			        This.SaveCustomData(myid, json);
			        This.setName(myid, newTxt, "line");
			    },
			    cancel: true,
			    init: function () {
			        $("#wkf_property .property_top li").each(function (index) {
			            var obj = $(this);
			            var son_property_div = $(".son_property_div .box");
			            obj.click(function () {
			                var lilist = obj.parent().find("li");
			                obj.addClass("on");
			                lilist.not(lilist[index]).removeClass("on");
			                son_property_div.eq(index).removeClass("hi");
			                son_property_div.not(son_property_div[index]).addClass("hi");
			            });
			        });
			        ...
			        $("#btnAddCondition").click(function () {
			            art.dialog({
			                title: "添加条件",
			                content: "<div>编号：<input type=\"text\" id=\"txtuno\" /> 条件：<input type=\"text\" id=\"txtuname\" /></div>",
			                lock: true,
			                ok: function () {
			                    var uno = $("#txtuno").val();
			                    ...
			                    if (uno != "" && uname != "") {
			                        var uobj = $("#u" + uno);
			                        if (uobj.length == 0) {
			                            var listr = "<li id=\"u" + uno + "\"><span uno=\"" + uno + "\">" + uname + "</span> <a href=\"javascript:void(0);\">删除</a></li>";
			                            $("ul.mylist").append(listr);
			                        }
			                    }
			                },
                            cancel:true
			        ...
			        //响应删除事件
			        $("ul.mylist li a").live("click", function () {
			            var dobj = $(this);
			            art.dialog({
			                title: "警告",
			                lock: true,
...
//			this.title=remarkHead[$(this).attr("id").split("btn_")[1]];
//		});
//		this.$nodeRemark=remark;
		this.$head.children("a").children("b").each(function(){
			this.title=remarkHead[$(this).attr("class").split("co_")[1]];
	...
	initWorkForNode:function(){
		...
		this.$workArea.delegate(".GooFlow_item","mouseup",{inthis:this},function(e){
			...
			if(testx == 2) {
				var This=e.data.inthis;
				//focusItemdocument.getElementById("heheda.").innerHTML = "通过innerHTML方法向页面输出了内容";
				//alert(this.type);
				if(This.$nowType!="direct")	return;
		...
		this.$workArea.delegate(".ico + td","dblclick",{inthis:this},function(e){
			...
			var tp = This.$nodeData[id].type;//取当前节点的类型，根据不同的类型，
			var content = "";
			var json = This.GetCumstomData(id);
			json.hjlx = tp;
			switch (tp) {
                //开始和结束是不需要有额外属性的，只需要简单的文字就行
            //开始和结束是不需要有额外属性的，只需要简单的文字就行
		    case "start"://开始
		        content = This.GetStartNodeContent(id, json);
		        break;			    	
		    case "end"://结束
		        content = This.GetEndNodeContent(id, json);
		        break;
		    case "task"://计算软件节点
		        content = This.GetCalculeContent(id, json);
		        break;
		    case "node"://可视化软件节点
		        content = This.GetNodeContent(id, json);
		        break; 
		    case "chat"://建模软件节点
		        content = This.GetchatContent(id, json);
		        break;
		        
			    case "state":
			        content = This.GetStartOrEndNodeContent(id, json);
			        break;
			    case "plug":
			        content = This.GetStartOrEndNodeContent(id, json);
			        break;
			...
			art.dialog({
			    ...
			    ok: function () {
			        ...
			        switch (tp) {
			            ...
			            case "complex":
			                var smstz = 0;
			                var txtsmstz = $("#txtsmstz");
			                if (txtsmstz.length > 0) {
			                    var smstzattr = txtsmstz.attr("checked");
			                    if (smstzattr == "checked" || smstzattr) {
			                        smstz = 1;
			                    }
			                }
			                var khq = 0;
			                var txtkhq = $("#txtkhq");
			                if (txtkhq.length > 0) {
			                    var khqattr = txtkhq.attr("checked");
			                    if (khqattr == "checked" || khqattr) {
			                        khq = 1;
			                    }
			                }
			                var kjs = 0;
			                var txtjslc = $("#txtjslc");
			                if (txtjslc.length > 0) {
			                    var jslcattr = txtjslc.attr("checked");
			                    if (jslcattr == "checked" || jslcattr) {
			                        kjs = 1;
			                    }
			                }
			                var ulli = $("ul.mylist li");
			                var sarr = [];
			                if (ulli.length > 0) {
			                    for (var i = 0; i < ulli.length; i++) {
			                        var curobj = ulli.eq(i).find("span");
			                        var uno = curobj.attr("uno");
			                        var uname = curobj.text();
			                        if (uno == undefined || uno == "") {
			                            continue;
			                        }
			                        var nj = {};
			                        nj.uno = uno;
			        ...
			        This.SaveCustomData(id, json);
			        This.setName(id, json.hjname, "node");
			    },
			    cancel: true,
			    init: function () {
                    //选项卡切换
			        ...
			        $("#btnAddUser").click(function () {
			            art.dialog({
			                title: "添加软件",
			                content: "<div>软件代号：<input type=\"text\" id=\"txtuno\" /> 软件名称：<input type=\"text\" id=\"txtuname\" /></div>",
			                lock: true,
			                ok: function () {
			                    var uno = $("#txtuno").val();
			                    var uname = $("#txtuname").val();
...
///////////以下为有关画线的方法
    ...
    //保存用户的自定义数据,字段包括：hjlx,hjname,smstz,khq,kjs,userlist,conlist
	SaveCustomData: function (id, data) {
	    var json = {};
	    json.id = id;
	    json.hjlx = data.hjlx;
	    json.hjname = data.hjname;
	    json.smstz = data.smstz;
	    json.kjs = data.kjs;
	    json.khq = data.khq;
	    json.userlist = data.userlist;
	    json.conlist = data.conlist;
	    //检测集合中是否有这个对象，有就修改，没有就添加
	    var mylen = this.$customData.length;
	    var ishas = false;
	    if (mylen > 0) {
	        for (var i = 0; i < mylen; i++) {
	            var ojson = this.$customData[i];
	            if (ojson.id == id) {
	                ojson.hjlx = data.hjlx;
	                ojson.hjname = data.hjname;
	                ojson.smstz = data.smstz;
	                ojson.kjs = data.kjs;
	                ojson.khq = data.khq;
	                ojson.userlist = data.userlist;
	                ojson.conlist = data.conlist;
	                ishas = true;
	                break;
	            }
	        }
	    ...
	    if (!ishas) {
	        this.$customData.push(json);
	    }
	},
    //通过ID取自定义数据
	GetCumstomData: function (id) {
	    var defjson = { id: id,  hjlx: "", hjname: "", smstz: 0, khq: 0, kjs: 0, userlist: null, conlist: null  };//初始一个空的记录
	    if (this.$customData.length == 0) return defjson;
	    var mylen = this.$customData.length;
	    for (var i = 0; i < mylen; i++) {
	        var json = this.$customData[i];
	        if (json.id == id) {
	            ishas = true;
	            return json;
	        }
    ...
    //生成普通节点的内容
	GetNormalNodeContent:function(id,json)
	{
	    var arr = [];
	    arr.push("<div id=\"wkf_property\">");
	    arr.push("                    <div class=\"property_top\">");
	    arr.push("                        <ul>");
	    arr.push("                            <li class=\"on\">基本信息</li>");
	    arr.push("                            <li>上传输入文件</li>");
	    arr.push("                        </ul>");
	    arr.push("                        <div class=\"clear\"></div>");
	    arr.push("                    </div>");
	    arr.push("                    <div class=\"son_property_div\">");
	    arr.push("                        <div class=\"box\">");
	    arr.push("                            <table width=\"100%\" cellpadding=\"0\" cellspacing=\"0\">");
	    arr.push("                                <tr>");
	    arr.push("                                    <td style=\"width: 120px\" class=\"ri\">节点编号：</td>");
	    arr.push("                                    <td><input type=\"text\" id=\"txthjbh\" readonly=\"readonly\" class=\"bg_gray\" value=\""+json.id+"\" /></td>");
	    arr.push("                                </tr>");
	    arr.push("                                <tr>");
	    arr.push("                                    <td class=\"ri\">节点类型：</td>");
	    arr.push("                                    <td><input type=\"text\" readonly=\"readonly\" id=\"txtnodetype\" class=\"bg_gray\" value=\"" + json.hjlx + "\" /></td>");
	    arr.push("                                </tr>");
	    arr.push("                                <tr>");
	    arr.push("                                    <td class=\"ri\">节点名称：</td>");
	    arr.push("                                    <td><input type=\"text\" id=\"txtHjname\" value=\"" + json.hjname + "\" /></td>");
	    arr.push("                                </tr>");
	    arr.push("                                <tr>");

	    //var smstz = json.smstz == 1 ? "checked=\"checked\"" : "";
	    //arr.push("                                    <td><input type=\"checkbox\" id=\"txtsmstz\" " + smstz + " /><label for=\"txtsmstz\">是</label></td>");
	    //arr.push("                                </tr>");
	    //arr.push("                                <tr>");
	    //arr.push("                                    <td class=\"ri\">自定义功能：</td>");
	    //var hq = json.khq == 1 ? "checked=\"checked\"" : "";
	    //arr.push("                                    <td><input type=\"checkbox\" id=\"txtkhq\" " + hq + " /><label for=\"txtkhq\">是</label></td>");
	    //arr.push("                                </tr>");
	    //arr.push("                                <tr>");
	    //arr.push("                                    <td class=\"ri\">可结束流程：</td>");
	    //var kjs = json.kjs == 1 ? "checked=\"checked\"" : "";
	    //arr.push("                                    <td><input type=\"checkbox\" id=\"txtjslc\" " + kjs + " /><label for=\"txtjslc\">是</label></td>");
	    //arr.push("                                </tr>");
	    arr.push("                            </table>");
	    arr.push("                        </div>");
	    arr.push("                        <div class=\"box hi\">");
	    arr.push("                            <input type=\"button\" value=\"选择输入文件\" id=\"btnAddUser\" />");
	    var userstr = "";
	    if (json.userlist != undefined && json.userlist != null) {
	        var sarr = [];
	        for (var i = 0; i < json.userlist.length; i++) {
	            sarr.push("<li id=\"u" + json.userlist[i].uno + "\"><span uno=\"" + json.userlist[i].uno + "\">" + json.userlist[i].uname + "</span><a href=\"javascript:void(0);\">删除</a></li>");
	        }
	        userstr = sarr.join("");
	    }
	    arr.push("                            <ul class=\"mylist\">" + userstr + "</ul>");
	    arr.push("                        </div>");
	    arr.push("                    </div>");
	    arr.push("                </div>");
	    var str = arr.join("");
    ...
    //生成开始节点的内容
	GetStartNodeContent: function (id, json) {
	    var arr = [];
	    arr.push("<div id=\"wkf_property\">");
	    arr.push("                    <div class=\"property_top\">");
	    arr.push("                        <ul>");
	    ...
	    arr.push("                                <tr>");
	    arr.push("                                    <td class=\"ri\">节点名称：</td>");
	    arr.push("                                    <td><input type=\"text\" id=\"txtHjname\" value=\"开始任务\" /></td>");
    ...
    //生成结束节点的内容
	GetEndNodeContent: function (id, json) {
	    var arr = [];
	    arr.push("<div id=\"wkf_property\">");
	    arr.push("                    <div class=\"property_top\">");
	    arr.push("                        <ul>");
	    ...
	    arr.push("                                <tr>");
	    arr.push("                                    <td class=\"ri\">节点名称：</td>");
	    arr.push("                                    <td><input type=\"text\" id=\"txtHjname\" value=\"结束任务\" /></td>");
    ...
    //生成计算软件结点的内容
	GetCalculeContent: function (id, json) {
	    var arr = [];
	    arr.push("<div id=\"wkf_property\">");
	    arr.push("                    <div class=\"property_top\">");
	    arr.push("                        <ul>");
	    ...
	    arr.push("                            <table width=\"100%\" cellpadding=\"0\" cellspacing=\"0\">");
	    arr.push("                                <tr>");
	    arr.push("                                    <td style=\"width: 100px\" class=\"ri\">节点编号：</td>");
	    arr.push("                                    <td><input type=\"text\" id=\"txthjbh\" readonly=\"readonly\" class=\"bg_gray\" value=\"" + json.id + "\" /></td>");
	    arr.push("                                </tr>");
	    arr.push("                                <tr>");
	    arr.push("                                    <td class=\"ri\">节点类型：</td>");
	    ...
	    arr.push("                                </tr>");
	    arr.push("                                <tr>");
	    arr.push("                                    <td class=\"ri\">计算软件：</td>");
	    arr.push("                                    <td><select type=\"text\" id=\"txtHjname\"<option></option><option>Vina</option><option>Lammps</option><option>babel</option><option>calculix.ccx</option><option>calculix.cgx</option></select></td>");
    ...
    //生成可视化软件结点的内容
	GetNodeContent: function (id, json) {
	    var arr = [];
	    arr.push("<div id=\"wkf_property\">");
	    arr.push("                    <div class=\"property_top\">");
	    arr.push("                        <ul>");
	    ...
	    arr.push("                                </tr>");
	    arr.push("                                <tr>");
	    arr.push("                                    <td class=\"ri\">可视化软件：</td>");
	    arr.push("                                    <td><select type=\"text\" id=\"txtHjname\"<option></option><option>Paraview</option><option>Calculix.cgx</option><option>Vmd</option></select></td></td>");
    ...
    //生成建模软件结点的内容
	GetchatContent: function (id, json) {
	    var arr = [];
	    arr.push("<div id=\"wkf_property\">");
	    arr.push("                    <div class=\"property_top\">");
	    arr.push("                        <ul>");
	    ...
	    arr.push("                                </tr>");
	    arr.push("                                <tr>");
	    arr.push("                                    <td class=\"ri\">建模软件：</td>");
	    arr.push("                                    <td><select type=\"text\" id=\"txtHjname\"<option></option><option>FreeCAD</option><option>GMSH</option><option>ANSY</option></select></td></td>");
    ...
    //生成线型节点的属性对话框
	GetLineNodeContent: function (id, json) {
	    var arr = [];
	    arr.push("<div id=\"wkf_property\">");
	    arr.push("                    <div class=\"property_top\">");
	    arr.push("                        <ul>");
	    ...
	    arr.push("                                </tr>");
	    arr.push("                                <tr>");
	    arr.push("                                    <td class=\"ri\">条件名称：</td>");
	    arr.push("                                    <td><input type=\"text\" id=\"txtHjname\" value=\"" + json.hjname + "\" /></td>");
	    arr.push("                                </tr>");
	    arr.push("                            </table>");
	    arr.push("                        </div>");
	    arr.push("                        <div class=\"box hi\">");
	    arr.push("                            <input type=\"button\" value=\"添加流转条件\" id=\"btnAddCondition\" />");
	    var conditionstr = "";
	    if (json.conlist != undefined && json.conlist != null) {
	        var sarr = [];
	        for (var i = 0; i < json.conlist.length; i++) {
	            sarr.push("<li><span uno=\"" + json.conlist[i].bh + "\">" + json.conlist[i].condition + "</span><a href=\"javascript:void(0);\">删除</a></li>");
	        }
	        conditionstr = sarr.join("");
	    }
	    arr.push("                            <ul class=\"mylist\">" + conditionstr + "</ul>");
	    arr.push("                        </div>");
	    arr.push("                    </div>");
	    arr.push("                </div>");
	    var str = arr.join("");
